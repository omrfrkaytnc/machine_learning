...
df = pd.read_csv('1 -) PYTHON PROGRAMMING FOR DATA SCIENCE/datasets/persona.csv')
df.head()
df.shape
df.info()
...
df.pivot_table(values="PRICE",index=["COUNTRY", 'SOURCE'],aggfunc="sum")
...
## yasin
#değişken ekleme
agg_df["customers_level_based"] = agg_df[["COUNTRY", "SOURCE", "SEX", "AGE_CATEGORY"]].apply("_".join, axis=1)
agg_df.customers_level_based.value_counts()
#list comp methoduyla işlem
new_persona = agg_df.groupby("customers_level_based").agg({"PRICE": "mean"})
new_persona.reset_index(inplace=True)
new_persona.customers_level_based.value_counts()
...
new_persona.groupby("SEGMENT").agg({"PRICE": ["mean", "max", "sum"]}, observed=False)
